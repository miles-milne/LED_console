...
void shift_reg(uint8_t shift_set, uint16_t ser_pin, uint16_t clk_pin, uint16_t latch_pin) {
...
void display_word_single(char word[], int delay, _Bool flash, uint16_t ser_pin, uint16_t clk_pin, uint16_t latch_pin) {
	...
	for (int k = 1; k < length; ++k) {
		...
		shift_reg(CHAR_TO_SEVEN_SEG[word[k]], ser_pin, clk_pin, latch_pin);
		HAL_Delay(delay);

		if (flash) {
			shift_reg(0xFF, ser_pin, clk_pin, latch_pin);
			HAL_Delay(delay/4);
		}
...
void display_word_multiple(char word[], int speed, uint16_t ser_pin, uint16_t clk_pin, uint16_t latch_pin, uint16_t seg_pins[4]) {
	...
	if (length == 1) {
		...
		shift_reg(CHAR_TO_SEVEN_SEG[word[0]], ser_pin, clk_pin, latch_pin);
		HAL_Delay(delay);
		...
		shift_reg(CHAR_TO_SEVEN_SEG[word[1]], ser_pin, clk_pin, latch_pin);
		...
		shift_reg(CHAR_TO_SEVEN_SEG[word[2]], ser_pin, clk_pin, latch_pin);
		...
		shift_reg(CHAR_TO_SEVEN_SEG[word[3]], ser_pin, clk_pin, latch_pin);
		...
		for (int offset = 0; offset <= length-4; ++offset) {
			...
			for (int count = 0; count < speed; ++count) {
					shift_reg(CHAR_TO_SEVEN_SEG[word[offset]], ser_pin, clk_pin, latch_pin);
					multi_seven_seg_set(1, seg_pins);
					HAL_Delay(delay);

					shift_reg(CHAR_TO_SEVEN_SEG[word[offset+1]], ser_pin, clk_pin, latch_pin);
					multi_seven_seg_set(2, seg_pins);
					HAL_Delay(delay);

					shift_reg(CHAR_TO_SEVEN_SEG[word[offset+2]], ser_pin, clk_pin, latch_pin);
					multi_seven_seg_set(4, seg_pins);
					HAL_Delay(delay);

					shift_reg(CHAR_TO_SEVEN_SEG[word[offset+3]], ser_pin, clk_pin, latch_pin);
					multi_seven_seg_set(8, seg_pins);
					HAL_Delay(delay);
				}
		}
